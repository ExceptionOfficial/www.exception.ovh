...
<table class="table table-bordered table-responsive">
	<thead class="table-head">
		<tr>
	        ...
	        <th>Heure</th>
	        <th>Lieu</th>
		<?php if(isset($_SESSION['type']) AND $_SESSION['type'] != "NEW") { ?>
	        <th>Prix</th>
	        <th>Action</th>
	    <?php } ?>
		</tr>
...
<?php
	...
	foreach($entries as $entry) {
		...
		echo "<tr>"."<td>".$entry['name']."</td>"."<td>".$entry['date']->format('d/m/Y')."</td>"."<td>".$entry['date']->format('H:i')."</td>"."<td>".$entry['location']."</td>";
		if(isset($_SESSION['type']) AND $_SESSION['type'] != "NEW") {
			echo "<td>".number_format($entry['member_price'], 2)." €</td>"."<td><a href=#>+</a></td>";
		}
		echo "</tr>";
	}
